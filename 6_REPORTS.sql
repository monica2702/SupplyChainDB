...
SELECT * FROM SCDB.PROGRAMS;
SELECT * FROM SCDB.PROGRAM_PART;
SELECT * FROM SCDB.PART;
--NUMBER OF PARTICULAR PART REQUIRED FOR A PROGRAM
SELECT PROG_ID, PART_ID, QTY_FACTOR FROM SCDB.PROGRAM_PART WHERE PROG_ID = '&PROGRAM_ID';
SELECT PROG_ID, PART_ID, QTY_FACTOR FROM SCDB.PROGRAM_PART WHERE PROG_ID = '&PROGRAM_ID' AND PART_ID = '&PART_ID';
-- TOTAL NUMBER OF PARTS IN A PROGRAM
SELECT PROG_ID, COUNT(*) AS NO_OF_PARTS FROM SCDB.PROGRAM_PART WHERE PROG_ID = '&PROGRAM_ID' GROUP BY PROG_ID;
--NUMBER OF PROGRAMS THAT NEED A PART WHOSE QUANTITY_FACTOR IS GREATER THAN 1
SELECT * FROM SCDB.PROGRAM_PART WHERE QTY_FACTOR > 1;


--SELECT ALL OUTGOING INVOICES
SELECT * FROM SCDB.OUTGOING_INVOICE;
--SELECT NUMBER OF OUTGOING PARTS(QUANTITY) IN THE BATCH
SELECT * FROM SCDB.OUTGOING_INVOICE WHERE OUT_INV_NO = '&OUT_INV_NO';
--SELECT NUMBER OF OUTGOING PARTS FOR ALL BATCHES FOR A PPID 
SELECT * FROM SCDB.OUTGOING_INVOICE WHERE PROG_PART_ID = '&PROG_PART_ID';
--SELECT NUMBER OF OUTGOING PARTS FOR A PPID FOR THE LATEST BATCH
SELECT * FROM SCDB.OUTGOING_INVOICE WHERE 
    OUT_INV_DATE = (SELECT MAX(OUT_INV_DATE)FROM SCDB.OUTGOING_INVOICE WHERE PROG_PART_ID = '&PROG_PART_ID');
...
SELECT PROG_PART_ID, SUM(QUANTITY) AS TOTAL_QTY_DISPATCHED FROM SCDB.OUTGOING_INVOICE 
...
SELECT * FROM SCDB.EMPLOYEE_PROGRAM;
--SELECT THE START AND END DATE OF AN EMPLOYEE FOR ALL PROGRAMS ASSOCIATED WITH HIM/HER
SELECT EMP_ID, PROG_ID, START_DATE, END_DATE FROM SCDB.EMPLOYEE_PROGRAM WHERE EMP_ID = '&EMPLOYEE_ID';
--SELECT EMPLOYEE_ID OF A PROGRAM MANAGER
SELECT * FROM SCDB.EMPLOYEE_PROGRAM WHERE DESIGNATION = 'PROGRAM MANAGER';
--TOTAL NUMBER OF EMPLOYEE IN A PROGRAM
SELECT * FROM SCDB.EMPLOYEE_PROGRAM WHERE PROG_ID = '&PROGRAM_ID';
SELECT PROG_ID, COUNT(*) AS NO_OF_EMPLOYEE FROM SCDB.EMPLOYEE_PROGRAM WHERE PROG_ID = '&PROGRAM_ID' GROUP BY PROG_ID;
--FINDING ALL ACTIVE EMPLOYEE PROGRAM AS OF TODAY 
SELECT * FROM SCDB.EMPLOYEE_PROGRAM WHERE END_DATE >= TRUNC(SYSDATE) OR END_DATE IS NULL;
--FINDING EMPLOYEE WHO ARE NOT ASSOCIATED WITH ANY PROGRAM AS OF TODAY
SELECT * FROM SCDB.EMPLOYEE_PROGRAM WHERE END_DATE < TRUNC(SYSDATE);


--APPROVAL_DETAILS
SELECT * FROM SCDB.APPROVAL_DETAILS;
--SELECT ALL APPROVAL_DETAILS FOR A PARTICULAR PART_ID
SELECT * FROM SCDB.APPROVAL_DETAILS WHERE PART_ID = '&PART_ID';
-- SELECT ALL PART APPROVAL DETAILS FOR A PARTICULAR SUPPLIER
SELECT * FROM SCDB.APPROVAL_DETAILS WHERE SUPP_ID = '&SUPP_ID';
-- SELECT ALL APPROVAL DETAILS FOR A PART FROM A PARTICULAR SUPPLIER
SELECT * FROM SCDB.APPROVAL_DETAILS WHERE PART_ID = '&PART_ID' AND SUPP_ID = '&SUPP_ID';
-- SELECT ALL APPROVAL DATE FOR A PART ID
SELECT PART_ID, APP_DATE FROM SCDB.APPROVAL_DETAILS WHERE PART_ID = '&PART_ID';
-- SELECT ALL SQE FOR A SUPPLIER
SELECT SUPP_ID, SQE_REP FROM SCDB.APPROVAL_DETAILS WHERE SUPP_ID = '&SUPP_ID';
-- SELECT APPROVAL COST FOR A PART
SELECT PART_ID, APP_STATUS, APP_COST FROM SCDB.APPROVAL_DETAILS WHERE PART_ID = '&PART_ID';
-- SELECT ALL APPROVED STATUS
SELECT * FROM SCDB.APPROVAL_DETAILS WHERE APP_STATUS = 'APPROVED';
-- SELECT ALL PENDING STAUS
SELECT * FROM SCDB.APPROVAL_DETAILS WHERE APP_STATUS = 'PENDING';
-- SELECT ALL APPROVAL IN A YEAR
SELECT * FROM SCDB.APPROVAL_DETAILS WHERE EXTRACT(YEAR FROM APP_DATE) = '&YEAR';

-- PURCHASE_ORDER
-- SELECT ALL PURCHASE_ORDER
SELECT * FROM SCDB.PURCHASE_ORDER;
-- SELECT A PARTICULAR PURCHASE_ORDER DATE FOR APPROVAL_ID
SELECT PART_APPR_ID, PO_DATE FROM SCDB.PURCHASE_ORDER WHERE PART_APPR_ID = '&PART_APPR_ID';
-- SELECT ITEM_QUANTITY FOR A PARTICULAR PART_APPR_ID
SELECT PART_APPR_ID, ITEM_QTY FROM SCDB.PURCHASE_ORDER WHERE PART_APPR_ID = '&PART_APPR_ID';
...
SELECT DISTINCT CUSTOMER_ID, CUSTOMER_NAME  FROM SCDB.PROJECT_BY_CUSTOMER WHERE PROGRAM_ID IS NULL;
...
SELECT 
    ...
    FROM 
        SCDB.PROJECT_BY_CUSTOMER
        WHERE PROGRAM_ID IS NOT NULL
            GROUP BY customer_id, CUSTOMER_NAME
            ORDER BY COUNT(*) DESC;
            
SELECT * FROM SCDB.APPROVAL_WITH_EMPLOYEE;
...
FROM SCDB.APPROVAL_WITH_EMPLOYEE 
...
SELECT APP_EMP_ID, EMP_NAME, COUNT(*) AS APPROVED FROM SCDB.APPROVAL_WITH_EMPLOYEE
WHERE STATUS = 'APPROVED'
GROUP BY APP_EMP_ID, EMP_NAME
ORDER BY APPROVED DESC;

--GET NUMBER OF DENIED PROJECT BY EMPLOYEE
SELECT APP_EMP_ID, EMP_NAME, COUNT(*) AS DENIED FROM SCDB.APPROVAL_WITH_EMPLOYEE
WHERE STATUS = 'DENIED'
GROUP BY APP_EMP_ID, EMP_NAME
ORDER BY DENIED DESC;

--GET NUMBER OF PENDING PROJECT BY EMPLOYEE
SELECT APP_EMP_ID, EMP_NAME, COUNT(*) AS PENDING FROM SCDB.APPROVAL_WITH_EMPLOYEE
WHERE STATUS = 'PENDING'
GROUP BY APP_EMP_ID, EMP_NAME
ORDER BY PENDING DESC;

--GET ALL EVAUATION STATS
SELECT * FROM SCDB.EVALUATION_STATS_EMPLOYEE;
...
SELECT * FROM SCDB.EVALUATION_STATS_SUPPLIER;


    
SELECT * FROM SCDB.PART_APPROVAL_SUPPLIER;
...
SELECT
    ...
    ASUP.PHONE_NO
FROM
    SCDB.PART_APPROVAL_SUPPLIER PAS 
    LEFT OUTER JOIN SCDB.ACTIVE_SUPPLIER ASUP
...
FROM SCDB.PART_APPROVAL_SUPPLIER 
...
SELECT * FROM SCDB.ACTIVE_SUPPLIER;

SELECT * FROM SCDB.INVENTORY_FLOW_STATUS;
...
SELECT * FROM SCDB.CUSTOMER_PROGRAM_PART;
...
FROM SCDB.CUSTOMER_PROGRAM_PART
    GROUP BY CUSTOMER_ID, CUSTOMER_NAME
    ORDER BY NO_OF_PARTS DESC
;

SELECT * FROM SCDB.CUSTOMER_PROGRAM_INVOICE;
