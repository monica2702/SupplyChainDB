...
CREATE VIEW PROJECT_BY_CUSTOMER AS 
    ...
    FROM SCDB.CUSTOMER C 
        LEFT OUTER JOIN SCDB.PROGRAMS P
...
CREATE VIEW APPROVAL_WITH_EMPLOYEE AS
    SELECT 
        ...
        FROM SCDB.APPROVAL_DETAILS A 
        LEFT OUTER JOIN SCDB.EMPLOYEE E 
...
CREATE VIEW EVALUATION_STATS_EMPLOYEE AS
    SELECT DISTINCT 
        A.APP_EMP_ID, 
        A.EMP_NAME , 
        NVL(X.APPROVED,0) AS APPROVED, 
        NVL(D.DENIED,0) AS DENIED, 
        NVL(P.PENDING,0) AS PENDING,
        T.EVALUATION_DURATION AS AVG_EVALUATION_TIME
    FROM SCDB.APPROVAL_WITH_EMPLOYEE A 
        LEFT JOIN (
        ...
        LEFT JOIN(
            SELECT APP_EMP_ID, EMP_NAME, ROUND(SUM(MONTHS_BETWEEN(STATUS_DATE,SUBMIT_DATE))) AS EVALUATION_DURATION FROM SCDB.APPROVAL_WITH_EMPLOYEE 
                WHERE STATUS != 'PENDING'
                GROUP BY APP_EMP_ID, EMP_NAME
...
CREATE VIEW APPROVAL_WITH_SUPPLIER AS
    SELECT 
        ...
        A.APP_DATE AS STATUS_DATE 
    FROM SCDB.APPROVAL_DETAILS A 
        LEFT OUTER JOIN SCDB.SUPPLIER S 
...
CREATE VIEW EVALUATION_STATS_SUPPLIER AS
    SELECT DISTINCT 
        S.SUPPLIER_ID, 
        S.SUPPLIER_NAME , 
        NVL(A.APPROVED,0) AS APPROVED, 
        NVL(D.DENIED,0) AS DENIED ,
        NVL(P.PENDING,0) AS PENDING 
    FROM SCDB.APPROVAL_WITH_SUPPLIER S 
    LEFT JOIN(
        SELECT SUPPLIER_ID, COUNT(*) AS APPROVED FROM SCDB.APPROVAL_WITH_SUPPLIER
            WHERE STATUS = 'APPROVED'
            GROUP BY SUPPLIER_ID
    ) A ON S.SUPPLIER_ID = A.SUPPLIER_ID
    LEFT JOIN(
        SELECT SUPPLIER_ID, COUNT(*) AS DENIED FROM SCDB.APPROVAL_WITH_SUPPLIER
            WHERE STATUS = 'DENIED'
            GROUP BY SUPPLIER_ID
    ) D ON S.SUPPLIER_ID = D.SUPPLIER_ID
    LEFT JOIN(
        SELECT SUPPLIER_ID, COUNT(*) AS PENDING FROM SCDB.APPROVAL_WITH_SUPPLIER
            WHERE STATUS = 'PENDING'
            GROUP BY SUPPLIER_ID
...
AS
    SELECT 
        ...
        AWS.SUBMIT_DATE AS SUBMIT_DATE,
        AWS.STATUS_DATE AS APPROVAL_STATUS_DATE 
    FROM SCDB.PART P 
        LEFT JOIN  SCDB.APPROVAL_WITH_SUPPLIER AWS
...
FROM 
    SCDB.SUPPLIER S LEFT JOIN SCDB.SUPPLIER_CONTACT SC 
...
AS
    SELECT 
        ...
        EMP.EMP_END_DATE,
        EMP.DESIGNATION
    FROM SCDB.PROJECT_BY_CUSTOMER P
        LEFT JOIN(
            SELECT 
                E.EMP_NAME ,
                EP.DESIGNATION,
...
SELECT 
        ...
        PROGP.OUT_MPW,
        PROGP.IN_MPW
FROM SCDB.PROJECT_BY_CUSTOMER PC
    LEFT JOIN(
        SELECT 
            PP.PROG_ID, 
            P.PART_ID, 
...
FROM SCDB.PROJECT_BY_CUSTOMER PC
    LEFT JOIN (
        ...
        FROM SCDB.PROGRAM_PART PP
            LEFT JOIN SCDB.OUTGOING_INVOICE O
